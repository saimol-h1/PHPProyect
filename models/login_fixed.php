...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($usuario) || empty($password)) {
        header('Location: ' . getUrl('root', 'index.php?action=login&error=empty'));
        exit();
    }
    ...
    if ($row = mysqli_fetch_assoc($result)) {
        ...
        if (md5($password) === $row['password']) {
            ...
            if ($row['tipo_usuario'] === 'administrador') {
                header('Location: ' . getUrl('root', 'index.php?action=servicios&login=success'));
            } else {
                header('Location: ' . getUrl('root', 'index.php?action=servicios&login=success'));
            }
            exit();
            ...
            header('Location: ' . getUrl('root', 'index.php?action=login&error=invalid'));
            exit();
        }
    } else {
    ...
    // Método no permitido
    header('Location: ' . getUrl('root', 'index.php'));
    exit();
}
